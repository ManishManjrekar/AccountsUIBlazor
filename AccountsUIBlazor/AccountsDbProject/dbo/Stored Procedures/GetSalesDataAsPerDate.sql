CREATE PROCEDURE [dbo].[GetSalesDataAsPerDate]
    @CreatedDate DATE
AS
BEGIN
SET NOCOUNT ON;
    ...
    INNER JOIN [dbo].[StockIn] AS si ON s.StockInId = si.StockInId WHERE CONVERT(DATE, s.CreatedDate) = @CreatedDate AND s.IsActive = 1;
END
